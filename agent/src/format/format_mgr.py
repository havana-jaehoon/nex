...
class FormatMgr(SingletonInstance):
    ...
    def _on_init_once(self):
        ...
        for format_file in glob.glob(f'{SystemInfoMgr().config_dir}/format/**/*.json', recursive=True):
            with open(format_file, 'r', encoding='utf-8') as f:
                json_data = json.load(f)
                format_tuple_list = self._find_format_with_top_parent_name(json_data)
                for format_tuple in format_tuple_list:
                    schem_def = SchemaParser.parse_json_to_schema(format_tuple[1])
                    format_id = FormatMgr._gen_format_id(format_tuple[0], format_tuple[1].get("name"))
