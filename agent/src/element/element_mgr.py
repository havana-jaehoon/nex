...
class ElementMgr(SingletonInstance):
    ...
    def _on_init_once(self, config_mgr: ConfigBaseMgr):
        self._logger = Logger()
    ...
    def getQueryHandlers(self) -> List[Tuple[str, Server_Dynamic_Handler, dict]]:
        ...
        for element in self._elements.values():
            ...
            return_list.append((element.id, handler, kwargs))
        return return_list

    async def getData(self, element_id: str, filters: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
        element = self._elements.get(element_id, None)
        if element:
            return await element.getData(filters)
        else:
            return pd.DataFrame()
