...
from command.bak.cmd_cfg import CmdConfig
from command.bak.cmd_process import CmdProcess, CmdIntervalProcess
    ...
    def _interval_proc(self):
        ...
        try:
            ...
            if self._cmd_cfg.inputs:
                df_list = InternalDataReqMgr.getReqMany(self._cmd_cfg.inputs, self._element_mgr.getData)
            ...
            if len(df_list) == len(self._cmd_cfg.outputs):
                for element_id, proc_df in zip(self._cmd_cfg.outputs, df_list):
                    if proc_df is None:
                        continue
                    self._element_mgr.setData(element_id, proc_df)
