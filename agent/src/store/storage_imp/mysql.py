...
class MysqlStorage(Storage):
    ...
    def __init__(self, storage_cfg: dict):
        ...
        if self._storage_cfg.get('dbType', '').upper() != StorageType.MYSQL:
            raise Exception(f'MysqlStorage : storage type is not Mysql')
        ...
        self._db: str = self._storage_cfg.get('name', '')
        self._param: dict = self._storage_cfg.get('param', {})
        self._orm_proc = OrmProc(StorageType.MYSQL, self._user, self._password, self._ip, self._port, self._db, **self._param)

    def _print(self) -> str:
        return (f'MysqlStorage('
                f'ip={self._ip}, port={self._port}, user={self._user}, password={self._password}, '
                f'name={self._db}, param={json.dumps(self._param)})')
